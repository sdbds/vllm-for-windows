...
# Header for kernel_selector.h: defines KernelKey + hash, opens the map.
# long long for type-id fields: vllm::ScalarType::id() returns a 64-bit value.
# is_zp_float is hardcoded false in every config for this (MoE) variant, but
# we keep it in the key so the selector stays structurally identical to the
# non-MoE one and the lookup function signature is unchanged.
...
# Footer when FP8 is NOT supported: keep the TORCH_CHECK guard.
...
# Footer when FP8 IS supported: unconditional lookup.
...
# One map entry.  {comma} is "," for every entry except the last.
...
def generate_new_kernels():
    ...
    # ------------------------------------------------------------------ #
    # Collect all entries needed for kernel_selector.h while also        #
    # writing the per-type .cu kernel files (unchanged logic).           #
    # ------------------------------------------------------------------ #
